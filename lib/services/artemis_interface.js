...
const getEncodeTable = () => {
  ...
  .then(response => {
    ...
    parsedResponse.forEach(function(record) {
     $('#record-table').append(
      `<tr>
        <td class='encode-row left-shifted-row'> ${ record.title   } </td>
...
const getPendingEncodes = () => {
  fetch('http://25.64.106.41:3200/api/v1/pending_encodes')
  .then(response => {
    return response.json();
  }).then(parsedResponse => {
    parsedResponse.forEach(function(record) {
     $('#pending-record-table').append(
      `<tr class='pending-row'>
        <td class='encode-row left-shifted-row'> ${ record.worker } </td>
        <td class='encode-row centered-row'> ${ record.title } </td>
        <td class='encode-row centered-row'> ${ record.series  } </td>
        <td class='encode-row centered-row'> ${ record.season  } </td>
        <td class='encode-row centered-row'> ${ record.episode } </td>
        <td class='encode-row centered-row'> ${ record.encode_age} </td>
       </tr>`
     );
    })
  })
...
export { getEncodeNumbers, getEncodesByDay, getEncodeTable, getPendingEncodes };
